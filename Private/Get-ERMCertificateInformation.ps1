    ...
    try {
        ...
        try {
            ...
            foreach($Certificate in $Certificates) {
                ...
                if(-not $HostnameMatch){
                    $Reason +=
                        "Certificate name does not match computer"
                }
                $MachineStore +=
                    [PSCustomObject]@{
                        Subject =
            ...
            $Errors +=
                "Unable to read LocalMachine certificate store: $($_.Exception.Message)"
        }
        ...
        try {
            ...
            if($LASTEXITCODE -eq 0) {
                $CertificateAuthority.Available =
                    $true
                $CertificateAuthority.Reachable =
                    $true
                $CertificateAuthority.Configuration =
                    $CAConfig
            }
        } catch {
            $Warnings +=
                "Enterprise Certificate Authority could not be detected."
        }
        ...
        try {
            ...
            $Warnings +=
                "Unable to determine certificate auto enrollment status."
        }
        ...
        if($WinRMCertificates.Count -eq 0){
            ...
            $Warnings +=
                "No valid certificate suitable for WinRM HTTPS was found."
        }
        ...
        Write-ERMLog `
            -Message "Certificate detection completed"
    }catch {
        Write-ERMLog `
            -Level Error `
